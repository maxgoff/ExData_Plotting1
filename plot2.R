# This program creates plot2.png from the dataset.
...
plot2 <- function( datafile = "../data/household_power_consumption.txt"){
    ...
    # create the plot per the specification and close the png device
    plot( x= strptime(paste(s3$Date, s3$Time), format="%Y-%m-%d %H:%M:%S"), y=as(s3$Global_active_power, "numeric")/500, type="l", xlab="", ylab="Global Active Power(kilowatts)")
